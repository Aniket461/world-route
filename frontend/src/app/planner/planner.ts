...
export class PlannerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.api.config().subscribe({
      next: (cfg) => {
        this.initMap(cfg.mapboxToken);
        this.route.queryParamMap.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((params) => {
          const tripId = params.get('trip');
          if (tripId) this.loadSavedTrip(tripId);
        });
      },
      error: () =>
        (this.error =
  ...
  openSaveDialog(): void {
    if (!this.result || !this.start || !this.end || this.saving || this.savedOk) return;
    if (!this.auth.token()) {
      void this.router.navigate(['/login'], { queryParams: { next: '/' } });
      return;
    }
    this.tripTitle = `${this.start.name} → ${this.end.name}`;
  ...
  confirmSaveTrip(): void {
    if (!this.result || !this.start || !this.end || this.saving) return;
    const title = this.tripTitle.trim() || `${this.start.name} → ${this.end.name}`;
    this.saving = true;
    this.savedOk = false;
    this.error = '';
    this.api
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          ...
          const body = err.error;
          this.error =
            (typeof body === 'string' ? body : body?.error) || 'Could not save trip.';
        },
      });
  }

  private loadSavedTrip(id: string): void {
    if (!this.auth.token()) {
      void this.router.navigate(['/login'], { queryParams: { next: `/?trip=${id}` } });
      return;
    }
    this.api.getTrip(id).subscribe({
      next: (trip) => {
        const payload = trip.payload;
        if (!payload?.result || !payload.start || !payload.end) {
          this.error = 'Saved trip data is incomplete.';
          return;
        }
        this.profile = payload.profile || trip.profile || this.profile;
        this.start = payload.start;
        this.end = payload.end;
        this.stops = payload.places || [];
        this.startQuery = payload.start.name;
        this.endQuery = payload.end.name;
        this.stopQuery = '';
        this.result = payload.result;
        this.savedOk = true;
        this.error = '';
        const apply = () => {
          this.drawRoute(payload.result);
          this.placeOptimizedMarkers(payload.result.ordered);
          this.map?.resize();
        };
        if (this.mapReady) apply();
        else this.map?.once?.('load', apply);
        if (this.isMobileLayout()) this.setMobileSheet(true);
      },
      error: () => {
        this.error = 'Could not open that saved trip.';
      },
    });
  }
